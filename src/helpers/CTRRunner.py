...
class CTRRunner(BaseRunner):
	...
	def evaluate(self, dataset: BaseModel.Dataset, topks: list, metrics: list) -> Dict[str, float]:
		...
		# 1) 规范化指标字符串：去空格，统一大写，把连字符变成下划线
		def norm(m):
			s = str(m).strip().upper().replace('-', '_')
			# 2) 过滤掉误传的 Top-K 指标（这些是 Top-K 任务才用的）
			if s in {'NDCG', 'HR', 'RECALL', 'PRECISION', 'MAP', 'MRR'}:
				return None
			# 3) 常见别名统一
			if s == 'LOGLOSS':
				s = 'LOG_LOSS'
		...
		norm_metrics = [nm for nm in (norm(m) for m in (metrics or [])) if nm]
		# 4) 如果没传或都被过滤，给个 CTR 默认：AUC + LOG_LOSS
		if not norm_metrics:
			norm_metrics = ['AUC', 'LOG_LOSS']

		return self.evaluate_method(predictions, labels, norm_metrics)
